...
The systematic differentiation between the categories `proximal', `distal', `remote' and `indefinite"=interrogative' among pronouns (and other pro"=forms; see \sectref{subsec:5-2-1}) is to some extent carried over to and partly overlapping with certain sets of adverbial pro"=forms. 
...
half-\textsc{mpl} here become.\textsc{pfv"=mpl} half-\textsc{mpl} \textsc{prt} Narey-\textsc{obl}  to  go.\textsc{pfv.pl}  \\
...
head \textsc{prt} \textsc{3sg.prox.obl=}in also plait-\textsc{cv} \textsc{3sg.prox.obl=}in also plait-\textsc{cv} \\
...
\glt `The field was up above and the sand and stream were down below.' (A:JAN082)
\end{exe}
...
\ex
\label{ex:7-8}
\gll so méeš hatí maǰí-e uṭik-í [bhun]  whaí ba uḍheew-í wháat-u\\
...
\ex
\label{ex:7-11}
\gll dúe xálaka ba [túuri] dhrúuk-a ǰe ɡéa  \\
other people \textsc{prt} down gorge-\textsc{obl}  down.to go.\textsc{pfv.pl} \\
...
\ex
\label{ex:7-12b}
\gll paṇáar-u táapaṛ thaní [awaaɡír] qaribán dúu thiáaḍ-a míil-a táapaṛ hín-u\\
...
\ex
\label{ex:7-13}
\gll ɡhoortsí bi so áa kučúru bi uḍheew-í [oóra]    šan-á so šay bi tanaám sanɡí yhaí šan-á, ma ba [phará] khilaí dharíit-u \\
Ghortsi also \textsc{def.msg.nom} one dog also flee.\textsc{cv} over.here roof-\textsc{obl} \textsc{dem.msg.nom} thing also \textsc{3pl.acc} with come.\textsc{cv} roof-\textsc{obl} \textsc{1sg.nom} \textsc{prt} yonder alone remain.\textsc{pfv"=msg}\\
...
\textsc{3sg.gen} \textsc{prt} five six brother.\textsc{pl} near stay-\textsc{3pl} \textsc{pst}\\
...
\ex
\label{ex:7-17b}
\gll [dhúura] áa ɣar-í učát ɣar-í eeṛáa piír hín-u \\
...
now \textsc{prt} \textsc{1sg.nom} \textsc{3sg.gen} strongmanship.\textsc{gen} story.\textsc{pl} do.\textsc{prs"=msg}\\
...
hunting \textsc{prt} \textsc{rem} time-\textsc{gen}  much-\textsc{f} \\
...
The most common calendrical adverbs (\tabref{tab:7-cal}) used deictically are synchronically non"=derived.
...
\textsc{1sg.nom} \textsc{3sg.acc} today eat-\textsc{1sg} \textsc{prt} morning-\textsc{obl} \textsc{prt} like.what become-\textsc{1sg}\\
...
afternoon \textsc{prt} Pashambi \textsc{prt} oak-\textsc{gen} load take-\textsc{cv} \textsc{1sg.nom} \textsc{prt} free \\
...
\label{subsec:7-1-6}
Sentence adverbs (\tabref{tab:7-sadv}) modify entire utterances. They may for instance specify the speaker's attitude toward the event referred to, or emphasize its truth"=value, as in (\ref{ex:7-48b}). 
...
\textsc{3msg.nom} \textsc{prt} really Khush.Shah.\textsc{gen} \textsc{def.msg.nom} uncle \\
...
foreleg-\textsc{pl} stretch.out.\textsc{cv} \textsc{prt} \textsc{3sg.obl} on head put-\textsc{cv} \textsc{prt} sit.down.\textsc{pfv"=msg}\\
...
\ex
\label{ex:7-61}
\gll lhooméi [teeṇíi mákar-a ǰhulí] [askúun-a baándi] kuhée díi nikhéet-i\\
...
The reason reading is also the usual when \textit{ǰhulí} takes a~Verbal Noun as its argument (\sectref{subsec:13-4-3}). In this abstract usage, another postpostion, \textit{baándi}, also in (\ref{ex:7-61}), a~loan from Pashto, is alternatively used, particularly in A.
...
\ex
\label{ex:7-63b}
\gll ak muṭ-á wée [trúu šúunɡ-am maǰí] so mhaás čhúuṇ-u \\
...
leopard \textsc{def} deodar.tree-\textsc{obl} under come.\textsc{cv} \textsc{prt} \\
...
\ex
\label{ex:7-66}
\gll karáaṛu [tíi túuri] bheš-í=n-u \\
...
remain.\textsc{pfv"=msg} \textsc{3sg.gen} \textsc{dem.msg.nom} water \textsc{3sg.acc} \textsc{prt}  fire-\textsc{obl} on put-\textsc{cv} \textsc{prt} log-\textsc{pl} give-\textsc{prs"=mpl } \textsc{3sg.acc} under   \\
...
\textsc{3sg.obl} \textsc{def.msg.nom} fruit collect-\textsc{cv} \textsc{prt} bag.\textsc{obl} in  put.\textsc{cv} take.down.\textsc{pfv"=msg }\\
...
\textsc{rem.msg.nom} hunter \textsc{prt} fear-\textsc{gen}  deodar.tree-\textsc{obl} up upward come.up.\textsc{pfv"=msg}\\
...
oh water \textsc{2sg.nom} \textsc{prt} pasture-\textsc{gen} from  come.down-\textsc{prs"=msg}  \\
\glt `Oh water, I see you are coming down from the high pasture.' (A:SHY047)
\end{exe}
\begin{exe}
\ex
\label{ex:7-73}
...
\ex
\label{ex:7-76}
\gll [eení ɡhooṣṭ-á šíiṭi] ma seé hín-u  \\
...
\ex
\label{ex:7-77}
\gll hazrát iisa=aleehisalaám eesé páand-a pharé bi-áan-u\\
...
child-\textsc{gen}  in.front.of good.\textsc{msg} work \textsc{prt} do-\textsc{oblg} \\
...
\textsc{1sg.nom} \textsc{prt} \textsc{prox} fattened she.goat.\textsc{pl} \textsc{2sg.gen} for keep-\textsc{prs"=msg=q}\\
...
\ex
\label{ex:7-87}
\gll malikc̣heétr thaní [ɡhróom-a the nhiáaṛa] áak c̣híitr hín-u \\
...
\textsc{3pl.gen} field \textsc{prt} \textsc{3pl.gen} house-\textsc{obl} from about three four kilometre-\textsc{pl} away.from be.\textsc{pst}  \\
