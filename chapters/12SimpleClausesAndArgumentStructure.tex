...
Like most IA languages, there is a~strict distinction between intransitive and transitive verbs in Palula. Almost without exception, a~verb stem is either intransitive or transitive and cannot be ambivalent or polyvalent as far as transitivity is concerned. There is, on the other hand, a~fairly productive valency"=changing morphology (as described in \sectref{sec:8-5}) by which a~stem can increase or decrease its valency. A verb's transitivity is primarily diagnosed on basis of absence/presence of ergative morphology in the perfective and the aspectual shift between accusative alignment and ergative alignment as far as verb agreement is concerned (see \sectref{sec:11-1}). 
...
The direct object in many transitive verbs of this type has a~patient"=of"=change role corresponding rather closely with the role of the subject of the typical intransitive verb (cf. \textit{mar-/mhaaré-} `die/kill', \textit{phooṭé-/phooṭá-} `break\textit{(itr)}/break\textit{(tr)}'} etc.):
...
Also some less prototypically transitive verbs conform to this pattern, e.g. those with experiencer"=subjects rather than agent"=subjects as in the verbs exemplified above:
