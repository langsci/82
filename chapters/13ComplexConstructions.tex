...
\spitzmarke{Clitical \textit{ee}-conjoining.} A postposed conjunction clicit \textit{ee} is primarily used to conjoin two noun phrases, as can be seen in examples (\ref{ex:13-1})--(\ref{ex:13-3}). It is cliticized to the right of the first constituent, following on any other word class specific inflections.
...
\spitzmarke{Sentential disjunction with \textit{yaába}{\ldots}\textit{yaába}.} Although there are few examples, it seems a~construction with a~repeated \textit{yaá} combined with the particle \textit{ba} (probably forming one phonological word), as in (\ref{ex:13-34}), is preferred for sentential disjunction, possibly with an~added emphasis on the mutual exclusivity of the alternatives thus presented. 
...
\ex
\label{ex:13-100}
\gll [ma róot-a ɡaɡalá ṭeem-íi yhayí ma  tu the ṭaak-úum seentá] tu
...
\ex
\label{ex:13-103}
\gll [inšaalaá heenṣúka paás bhíl-a heentá]  ɡhróom-a iskuul-í the wh-áaya \\
...
\ex
\label{ex:13-106}
\gll [maṇḍáu kareé thíil-u heentá] maṇḍáu wée  ɡhareé ḍhínɡar ɡhooṣṭ-á díi tuúš muxtalíf bh-áan-u  \\
...
Apart from the explicitly marked Conditional clauses exemplified above, we also find conditionality implied in Converb clauses, as in (\ref{ex:13-110}), as well as in \textit{ta}-clauses, as in (\ref{ex:13-111}). The Past Imperfective in the `then'-clause in (\ref{ex:13-111}) implies unfulfilment by itself.
...
companion-\textsc{pl.obl} to say-\textsc{1sg} \textsc{ pst} \textsc{prt} corpse throw-\textsc{cv} flee-\textsc{3pl} \textsc{ pst} \\
...
\textsc{1sg.nom} \textsc{3sg.prox.acc} today eat-\textsc{1sg} \textsc{prt} morning-\textsc{obl} \textsc{prt} like.what become-\textsc{1sg} \\
...
\ex
\label{ex:13-113}
\gll [yáraa ma islaamabaad-á ɡáu de  heentá] šuy bhíl-u de heentá,
...
\ex
\label{ex:13-192}
\gll [kasée ɡalé maáṭu dhraǰaá ba šúul-a the  phedíl-u heentá] hasó kh-úu \\
...
\ex
\label{ex:13-193}
\gll [anú íṇc̣-a sanɡí mháala kií ɡalá ɡhašíl-i  heentá] ma tas
