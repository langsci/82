...
\tabref{tab:8-29}. The verbs derived in this way all belong to the \textit{a}"=ending L"=class. The valency"=increasing suffix added to an~intransitive imperfective stem \textit{(núuṭ-)} thus derives a~transitive imperfective verb stem \textit{(nuuṭá-)}, and when added to an~already transitive stem \textit{(the-)}, a~causative verb is derived \textit{(thawá-)}. As this process is a~matter of stem formation, any inflectional morphology, as described above, occurs after the derivational suffix: \textit{pašáan-a} `are seeing' {\textgreater} \textit{pašawa-(y)áan-a} /paɕaˈwajna/ `are showing', as is also apparent from the examples below.
...
\ex
\label{ex:8-26}
\gll bhun whaí ba [uḍheew-í wháat-u] \\
