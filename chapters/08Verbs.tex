...
The two verbal communication verbs \textit{thané-} and \textit{mané-} interact in a~language"=specific way and are, beyond their simple characteristic as utterance"=verbs that take direct"=quote complements \citep[155]{givon2001a}, also grammaticalised in one or more forms as quotatives and hearsay markers, respectively, see \sectref{subsec:9-2-4} and \sectref{subsec:13-5-1}.\footnote{This, however, is not reflected to any greater extent in this frequency count where only finite verb forms are taken into account.}
...
This is one of the largest subclasses, and~-- again~-- the verbs listed in \tabref{tab:8-la} are only a few examples.
...
They are also (\tabref{tab:8-ti}), like the previous group, in some sense motion verbs, at least three of them describing upward motion. Two of them are transitive and two intransitive (including the already exemplified \textit{uthí-}).
...
\tabref{tab:8-15}, with \textit{til-} `walk' as an~example verb, gives an~overview of verbal inflectional categories, including finite as well as non"=finite forms.
...
\textbf{Imperative.} All verbs form distinct singular and plural imperatives, for example \textit{čhín} \textsc{(sg)}~-- \textit{čhínooi} (\textsc{pl}) `cut!' More examples are provided in \tabref{tab:8-22}.
...
The temporal versus aspectual character the TMA"=category Present, using this particular verb form, is further discussed under Section \sectref{subsec:9-1-3}.
...
&
\multicolumn{2}{ l}{Stem derived by valency addition}\\\hline
\textit{buǰ-} &
`understand' &
...
There are also some non"=productive (or irregular) derivations (\tabref{tab:8-irradd}), reflecting historical or alternative patterns, with root vowel alternation, aspiration alternation, and a~derivative segment \textit{-aal}. 
...
Although this, strictly speaking, is not part of our focus here, we can see that the same construction is used regardless of the degree of voluntary control that the causee exercises: compare (\ref{ex:8-7}) with (\ref{ex:8-8}), (\ref{ex:8-9}) with (\ref{ex:8-10}), and (\ref{ex:8-11}) with (\ref{ex:8-12}).
...
\multicolumn{2}{ l}{Non"=derived stem} &
&
\multicolumn{2}{ l}{Stem derived by valency reduction}\\\hline
\textit{bilá-} &
`melt' (\textit{tr}) &
\centering {\textgreater} &
\textit{bil-íǰ-} &
...
eight=twenty-\textsc{pl} year-\textsc{pl} after \textsc{1sg.gen} hand-\textsc{obl} \textsc{prt} \textsc{3fsg.nom} be.buried.\textsc{pfv-f}\\
...
\ex
\label{ex:8-19}
\gll tíi /{\ldots}/ tanaám bíiḍ-a [tanɡ thíil-a hín-a] \\
...
\ex
\label{ex:8-22}
\gll kuṛíina ta támbul"=am"=ii ǰ-íin [rhoo-á d-íin] \\
woman.\textsc{pl} \textsc{prt} drum-\textsc{pl.obl"=gen} beat-\textsc{3pl} song-\textsc{pl} give-\textsc{3pl}\\
