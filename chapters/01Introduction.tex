...
The most visible modifications made to the existing Urdu alphabet in order to write Palula have to do with symbols representing a number of retroflex sounds: <ڇ> for \textit{c̣}, <ڙ> for \textit{ẓ}, <ݜ> for \textit{ṣ}, and <ݨ> for \textit{ṇ}. To write \textit{ts}, the symbol <څ> has been borrowed from the Pashto alphabet. Aspiration (as in \textit{ph, th, čh}) as well as \textit{h} occurring in clusters with voiced consonants (as in \textit{dh, mh, bhr}) are represented by <ھ> (see \tabref{tab:1-4bb}).
...
Another modification relates to the representation of the ten vowels. Because of the many crucial contrasts between long and short vowels, particularly in word-final position, the developers of the Palula orthography introduced a language-particular use of a diacritic <ۡ> to mark a short vowel, as displayed in \tabref{tab:1-4c}. Mainly due to challenges in the area of font development, Palula materials are exclusively produced in naskh style, not in nasta’liq.
