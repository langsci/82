...
Examples: \textit{utráp-} (\textsc{itr}) `to run', \textit{mar-} (\textsc{itr}) `to die', \textit{ɡhin-} (\textsc{tr}) `to take, buy', \textit{phaalé-} (\textsc{tr}) `to break, tear', \textit{bhe-} (\isi{copula}, \textsc{itr}) `to become; to come into existence'.\footnote{The citation forms with a final hyphen are underlying \isi{imperfective} verb stems.}
