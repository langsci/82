...
The first- and second"=person singular pronouns on the other hand make fewer distinctions than most full nouns, with their two"=way case contrast. As in \ili{Kohistani} \ili{Shina}, the \isi{oblique} case has merged with the \isi{nominative} \citep[82]{schmidtkohistani2008}. The \isi{nominative} is thus used for in\isi{transitive} subjects, as the \textsc{1sg} \textit{ma} in (\ref{ex:5-3}), all direct objects, as the \textsc{1sg} \textit{ma} in (\ref{ex:5-5}), as well as for \isi{transitive} agents in the im\isi{perfective}, while the \isi{genitive} covers the possessor of a~possessive construction, as the \textsc{1sg} \textit{míi} in (\ref{ex:5-3}), as well as the agent of \isi{perfective} clauses, as exemplified by \textsc{1sg} \textit{míi} in (\ref{ex:5-4}). 
...
However, in uses with \isi{nominative} \isi{pronominal} forms, as in (\ref{ex:5-9}), it takes on a~meaning similar to `I, myself, etc'. Note that this, in Palula, is a~function distinct from that of the reflexive pronouns (see \sectref{sec:5-4}).
...
Also a~reduplicated form of a~\isi{pronoun} and the suffix, as in (\ref{ex:5-10})--(\ref{ex:5-11}), is used in approximately the same sense.
...
As mentioned in the introduction to this chapter, the \isi{demonstrative} pronouns are part of a~larger system of pro"=forms (pro"=NPs, pro"=adverbs, pro"=adjectives, etc.), displaying some degree of symmetry (\tabref{tab:5-2}). Most (but not all) of those pro"=forms have similar"=looking sets consisting of a~\isi{proximate}, a~\isi{distal}, a~\isi{remote} and an~inde\isi{finite}/\isi{interrogative} member (see \citealt[187--188]{haspelmath1993} for some strikingly similar sets in Lezgian).
...
Remote
&
Inde\isi{finite}/{\allowbreak}\isi{interrogative}\\\hline
Attributive &
\textit{anú, eenú} &
\textit{aṛó, eeṛó} &
\textit{eesó} &
...
\textit{eetáa} &
\textit{ɡóo} \\
Location II\footnote{It is only with a~following \isi{postposition} that these forms have a~locational interpretation. Used alone they (ergativily) code the \isi{subject} of a~\isi{perfective} \isi{transitive} \isi{clause}.} &
\textit{aníi=} &
\textit{eeṛíi=} &
\textit{eetíi=} &
\textit{kíi=} \\
...
As can be seen in \tabref{tab:5-3}, the basic adnominal demonstratives occur in only two different forms in each \isi{deictic} set, as compared to the much more complex system of \isi{pronominal} demonstratives that we will discuss shortly. 
...
Non"=\isi{nominative}/{\allowbreak}Plural/{\allowbreak}Feminine\\\hline
Proximate &
...
\tabref{tab:5-1}, are number, case, \isi{gender} and distance/visibility, the first three also found with Palula
nouns.
...
The masculine"=feminine \isi{gender} distinction is found only in the singular \isi{nominative}, cf. (\ref{ex:5-13}) and (\ref{ex:5-14}), whereas masculine and feminine are syncretized in the plural, as is evident when comparing (\ref{ex:5-15}) with (\ref{ex:5-15b}). 
...
The pronouns in the weak \isi{remote} sets of the demonstratives (\textit{so}, \textbf{etc.}), are the default choice and most frequently used as typical third person pronouns, with easily accessible \isi{discourse} referents \citep[432--433]{diessel2006}, and could be said to stand particularly close functionally to the personal pronouns proper. 
...
The weak"=strong opposition is related to accessibility. The strong forms tend to be picked for \isi{deictic} or \isi{anaphoric} use in order to keep track of less accessible \isi{discourse} referents, or to contrast another referent or switch from one referent to another. They are also the ones used in relative"=correlative clauses and in most cases as adnominal demonstratives. 
...
most natural or accessible referent in a~\isi{discourse}. In example (\ref{ex:5-17}) the antecedent of the \isi{pronoun}
...
There is also another continuum that cuts across the entire paradigm. While the \isi{remote} category is primarily a~phoric device \citep[131]{saxena2006}, with a~high frequency in, for instance, narrative \isi{discourse}, and only secondarily functions deictically, the opposite holds for the proximal category, which is primarily \isi{deictic} and only rarely functions phorically. The mid"=level category, the \isi{distal}, is used extensively in both functions. 



An indirect effect of the differences in \isi{deictic} scope is the ability to signal a~contrast in the presence"=absence of the third"=person object or person in the speech situation. It is normally only when a~person or object is present in the speech situation that the proximal and \isi{distal} terms are used (exophorically or anaphorically), whereas the \isi{remote} assumes its absence. The same holds for a~distinction between narration and direct speech - especially crucial in oral story"=telling. Along with other \isi{quotative} devices, the proximal and \isi{distal} terms refer to the outside world as perceived by the quoted speaker, whereas the narrator (whether identical to the former or not) would use the corresponding \isi{remote} terms to refer to the same entities.



\citet[204--205, 207--212]{schmidt2000} and \citet[134--136]{schmidtkohistani2001} describe similar effects obtained in the use of different sets of pronouns in some other \ili{Shina} varieties. In Tileli there are two sets of third"=person pronouns, where one set signals visibility and first"=hand knowledge, whereas the other one codes what is invisible, unknown or indirectly inferred. In \ili{Kohistani} \ili{Shina}, a~similar discrimination can be made between information derived by visual means and information received by other means, through contrasting a~\isi{proximate} set of demonstratives with a~\isi{remote} set. 



Three particular functions (\isi{exophoric}, \isi{anaphoric} and \isi{discourse}"=\isi{deictic}) will be further discussed and exemplified below (\sectref{subsec:5-2-3}--\sectref{subsec:5-2-5}), following the pragmatic taxonomy of demonstratives suggested by \citet[205--254]{himmelmann1996} and discussed by \citet[432]{diessel2006}. I do not have any clear examples of recognitional use, a~fourth category; however, there is evidence for the development of a~de\isi{finite}, or ``\isi{anaphoric} article'' \citep[486]{juvonen2006}, contrasting with a~similarly developing inde\isi{finite} article. These two will be discussed in \sectref{subsec:5-2-6}. A secondary spatial specification will also be touched upon briefly (\sectref{subsec:5-2-7}).
...
\ex
\label{ex:5-22}
\gll [aní dees-óom] atsareet-á wée qariibán čúur zára kušúni
hín-a \\
...
\ex
\label{ex:5-25}
\gll [aṛó míiš] thíi khaṇíit-u ɡa heentá ma tu the bíiḍ-i inaám d-úum \\
...
\end{exe}
It is probably futile to try and give absolute measures of when the proximal term is used as compared to when the \isi{distal} term is used. It is more a~matter of relative distance, as perceived by the speaker/experiencer in the situation. There are no apparent animacy restrictions on any of these terms when used exophorically, which seems to be the case with the corresponding terms in, for instance, \ili{Kohistani} \ili{Shina} \citep[135]{schmidtkohistani2001}.

In as far as the \isi{remote} term is being used exophorically, it is in referring to an entity that is \textit{not} present or visible in the utterance situation. 
...
\subsection{Discourse"=\isi{deictic} use}
\label{subsec:5-2-5}

The \isi{discourse}"=\isi{deictic} use is to do with entire propositions being referenced rather than with tracking individual \isi{discourse} participants. Its function is to combine chunks of \isi{discourse} \citep[432]{diessel2006}, and is thus more abstract in its scope.



In this rather specialised function, it seems the \isi{distal} term is used most of the time. It may be either anticipatory, as in (\ref{ex:5-30}), or refer back to a~rather long preceding chunk of \isi{discourse} as in (\ref{ex:5-31}).
...
\end{exe}
Although the typical use seems to be \isi{pronominal}, it also occurs adnominally, as in (\ref{ex:5-32}).
\begin{exe}
\ex
\label{ex:5-32}
...
\end{exe}
It usually occurs in its strong form, but it may occasionally be less emphasised, as in (\ref{ex:5-33}), where the weak form refers back to the preceding proposition, thus ``naming'' what has already been defined through \isi{discourse}.
\begin{exe}
\ex
\label{ex:5-33}
...
\end{exe}
The same `plan' or `consultation' is a~little later, in (\ref{ex:5-35}), referred to with the \isi{distal} term, then from the perspective of a~secret listener, standing outside the house where the exchange was taking place.
\begin{exe}
\ex
\label{ex:5-35}
...
\end{exe}
The \isi{remote} term does not seem to be frequently used \isi{discourse}"=deictically. A possible example may be the one in (\ref{ex:5-36}), where the \isi{demonstrative} refers to a~point in time in a~narrative, the `night' following the events described in the immediately preceding chunk of \isi{discourse}.
\begin{exe}
\ex
\label{ex:5-36}
...
\end{exe}
However, this rather specialised function, \citet[225]{himmelmann1996} defines as a~subtype of \isi{discourse}"=\isi{deictic} use.

Sometimes there is also an~ambiguity as to whether the \isi{demonstrative} refers anaphorically to a~particular \isi{discourse} referent or to a~preceding chunk of \isi{discourse}:
...
Here, the plank as well as the totality are possible referents for the \isi{demonstrative}.

\subsection{Article"=like uses}
\label{subsec:5-2-6}

Closely related to adnominally used demonstratives, and still possibly a~subcategory of them, are the forms \textit{so} and \textit{se} (formally the weak form masculine and feminine \isi{pronominal} demonstratives of the \isi{remote} set), used for signalling de\isi{finite} or previously introduced and firmly established entities in the \isi{discourse}. Their distribution is the same as that of the adnominal demonstratives: the form \textit{so} is used preposed to a~masculine singular \isi{nominative} head, whereas \textit{se} is used with a~head in any other case, \isi{gender} or number. 


\begin{table}[ht]
 \caption{De\isi{finite} ``articles''}
\begin{tabularx}{\textwidth}{ Q Q }
\lsptoprule
Nominative masculine singular head &
Non"=\isi{nominative} / Plural / Feminine head\\\hline
\textit{so} &
...
Contrasting in definiteness and identifiability with \textit{so/se} used in this way, is the ``inde\isi{finite} article'' \textit{áa} or \textit{áak} (B. \textit{a} or \textit{ak}), derived from, and in one of its forms still homonymous with, the \isi{numeral} \textit{áak} `one'.
...
While \textit{áa} is used for introducing a~participant, \textit{áa míiš} `a (certain) man', \textit{so/se} is used in order to refer to or track an~already earlier introduced participant, \textit{so míiš} `the/that man'.
  ...
  (\ref{ex:5-38}), the man is introduced, with a~full \isi{noun} and the inde\isi{finite} \textit{áa}.
...
introduced by a~full \isi{noun} preceded by the inde\isi{finite} \textit{áa} and a~descriptive attribute,
`hairy'.
\begin{exe}
\ex
\label{ex:5-39}
...
between the inde\isi{finite} and the de\isi{finite}), while \textit{míiš} `man', when he is referred to
again, is done so through the use of a~full \isi{noun} marked with the de\isi{finite} \textit{so}.
\begin{exe}
\ex
\label{ex:5-40}
...
In the same way, when the monster is referred to in the following sentence, in (\ref{ex:5-41}), after the man is mentioned, he too is referred to by using a~full \isi{noun} and the de\isi{finite} \textit{se} (that form because the head is in the \isi{oblique} case).
\begin{exe}
\ex
\label{ex:5-41}
...
Having exemplified this use of the ``de\isi{finite} article'' primarily as a~tracking device, it should be pointed out that there are no clear and unambiguous examples of it being used as identifiable in the larger situation or associative"=anaphorically, two uses that are typical for de\isi{finite} articles, while not generally associated with demonstratives in general (\citealt[485]{juvonen2006}; \citealt[233]{himmelmann1996}). Therefore, since the use of adnominal \textit{so/se} is not dramatically different from the \isi{anaphoric} use of adnominal demonstratives, an~alternative labelling of it would be ``\isi{anaphoric} article'' \citep[486]{juvonen2006}, but it also comes close to what \citet[230--239]{himmelmann1996} includes in the recognitional use of demonstratives. What sets it apart from the (strong) \isi{anaphoric} use (see \sectref{subsec:5-2-4}), however, and thus points to it being further grammaticalised towards a~more typical de\isi{finite} article interpretation as ``identifiable'' in general \citep[485]{juvonen2006}, is its frequent and near"=obligatory presence with already introduced and well"=established referents in a~\isi{discourse}, its weak, thus phonetically reduced, form and its paradigmatic contrast with the inde\isi{finite} article.


Along the same lines, the use of \textit{áa/áak} exemplied above does not set it apart as radically estranged from its use as a~\isi{numeral}, but it could be said to have gone through an~initial grammaticalisation stage (as a~phonetically reduced presentative marker) towards a~full"=blown inde\isi{finite} article \citep[486]{juvonen2006}.
...
\section{Inde\isi{finite}"=\isi{interrogative} pronouns}
\label{sec:5-7}


The area of inde\isi{finite} pronouns and their distribution needs further research, but there are strong indications that most, if not all, inde\isi{finite} pronouns are also used as \isi{interrogative} pronouns (as further exemplified in \sectref{subsec:14-2-2}). As mentioned above (\sectref{subsec:5-2-1}), the inde\isi{finite}"=\isi{interrogative} pronouns belong together with the demonstratives in the larger system of pro"=forms. An animacy distinction is also made here, cf. (\ref{ex:5-53}) and (\ref{ex:5-54}), not otherwise part of the \isi{pronominal} system.
...
Chitrali-\textsc{pl} \textsc{1pl.acc} to Dangarik call-\textsc{prs"=mpl} some \textsc{prt} \textsc{1pl.acc} to \ili{Kalasha}-\textsc{pl} call-\textsc{prs"=mpl}\\
\glt `The Chitralis call us Dangarik, and some consider us \ili{Kalasha}.' (A:ANJ015)
...
See also \sectref{subsec:14-3-2} about a~possible development of a~set of negative inde\isi{finite} pronouns (\textit{ɡá=bi=na} `nothing', \textit{koó=bi=na} `nobody', \textit{ɡóo=bi=na} `nowhere'), based on the basic inde\isi{finite} ones.

\section{Relative pronouns}
\label{sec:5-8}


There are no distinct relative pronouns in Palula. Instead the demonstratives and the inde\isi{finite}"=\isi{interrogative} pronouns are used in relative constructions, or what is the functional equivalent of relative clauses (see \sectref{sec:13-6}).