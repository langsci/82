...
Remote
&
Inde\isi{finite}/{\allowbreak}\isi{interrogative}\\\midrule
Attributive &
\textit{anú, eenú} &
\textit{aṛó, eeṛó} &
\textit{eesó} &
...
\citet[204--205, 207--212]{schmidt2000} and \citet[134--136]{schmidtkohistani2001} describe similar effects obtained in the use of different sets of pronouns in some other \iliShina varieties. In Tileli there are two sets of third"=person pronouns, where one set signals visibility and first"=hand knowledge, whereas the other one codes what is invisible, unknown or indirectly inferred. In \iliKohistani \iliShina, a~similar discrimination can be made between information derived by visual means and information received by other means, through contrasting a~proximal set of demonstratives with a~\isi{remote} set. 
...
Contrasting in definiteness and identifiability with \textit{so/se} used in this way, is the ``inde\isi{finite} article'' \textit{áa} or \textit{áak} (B \textit{a} or \textit{ak}), derived from, and in one of its forms still homonymous with, the \isi{numeral} \textit{áak} `one'. While \textit{áa} is used for introducing a~participant, \textit{áa míiš} `a (certain) man', \textit{so/se} is used in order to refer to or track an~already earlier introduced participant, \textit{so míiš} `the/that man'. The examples (\ref{ex:5-38})--(\ref{ex:5-41}) are all taken from one and the same story, where the two main characters are a~man and a (male) monster. At the very beginning of the story, in (\ref{ex:5-38}), the man is introduced, with a~full \isi{noun} and the inde\isi{finite} \textit{áa}.
...
\begin{table}[ht]
\caption{Secondary spatial specifications of \isi{distal} demonstratives}
...
\begin{table}[H]
\caption{Inde\isi{finite}"=\isi{interrogative} pronouns}
...
See also \sectref{subsec:14-3-2} for a~possible development of a~set of negative inde\isi{finite} pronouns (\textit{ɡá=bi=na} `nothing', \textit{koó=bi=na} `nobody', \textit{ɡóo=bi=na} `nowhere'), based on the basic inde\isi{finite} ones.

\section{Relative pronouns}
\label{sec:5-8}


There are no distinct relative pronouns in Palula. Instead, the demonstratives and the inde\isi{finite}"=\isi{interrogative} pronouns are used in relative constructions, or in the functional equivalents of relative clauses (see \sectref{sec:13-6}).