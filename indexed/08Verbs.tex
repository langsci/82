...
\begin{table}[t]
\caption{Palula Verbs Top Twenty. The twenty most frequent verbs. (The percentage is calculated on occurrence of \isi{finite} verb forms in the text corpus.)}
...
Two small groups of T-verbs can be described as hybrids between consonant"=ending and \textit{e}-ending verbs. The verbs in one of them (\tabref{tab:8-tas1}) behave like the consonant"=ending verbs in the im\isi{perfective} and like the \textit{e}-ending verbs in the \isi{perfective}.


\begin{table} 
\caption{Examples of \isi{accent}"=shifting T"=verbs, type 1}
...
The verbs of the other group (\tabref{tab:8-tas2}) look like \textit{e}-ending verbs in the im\isi{perfective} but like consonant"=ending verbs in the \isi{perfective}. 


\begin{table} 
\caption{Examples of \isi{accent}"=shifting T"=verbs, type 2}
...
Another subclass of equal size is one ending (at least underlyingly) with the vowel \textit{i}, as seen in \tabref{tab:8-11}. They are also (\tabref{tab:8-11b}), like the previous group, in some sense motion verbs, at least three of them describing upward motion. Two of them are \isi{transitive} and two in\isi{transitive} (including the already exemplified \textit{uthí-}).
...
basically any verb stem (\tabref{tab:8-27}). It seems that it in most cases simply substitutes for the vowels in vowel"=ending stems. This exemplifies a~verb form that seems to ``fall between two stools'' as far as finiteness is concerned (see \sectref{subsec:9-2-3} for a~further discussion). \citeauthor{schmidt2003}'s (\citeyear[139]{schmidt2003}) ``injunctive'' in \iliKohistani \iliShina, with its invariant form \textit{-óoṇṭha} and its similar semantics, is probably related to the Palula obligative.
...
As a~great number of verbs in the \textit{a}-ending class are either inherently \isi{transitive} (without any apparent in\isi{transitive} counterpart that it has been derived from) or are derived transitives, the final \textit{-á} has in itself become a~transitivity marker of some sort. The \isi{causative} or \isi{valency}"=increasing formation, as described above, can apply to in\isi{transitive} and \isi{transitive}, as well as stems already derived by the valancy"=increasing formation. 


\subsubsection*{Transitives derived from in\isi{transitive} verbs}

First, the formation can be applied to in\isi{transitive} verbs. Although this, strictly speaking, is not part of our focus here, we can see that the same construction is used regardless of the degree of voluntary control that the causee exercises: compare (\ref{ex:8-7}) with (\ref{ex:8-8}), (\ref{ex:8-9}) with (\ref{ex:8-10}), and (\ref{ex:8-11}) with (\ref{ex:8-12}).
...
While all the verb stems thus regularly derived causatively end up in one and the same morphological class (\tabref{tab:8-addcl}), namely the \textit{a}-ending L-class, the corresponding in\isi{transitive} verb could come from virtually any \isi{verb class}, although the most common source is the consonant"=ending class of L-verbs.


\begin{table} 
\caption{Valency addition and \isi{verb class} membership}
...
\iliChitral-\textsc{obl} to also phone do.\textsc{pfv"=f} \textsc{pst}\\
\glt `I also phoned \iliChitral.' (A:CHN070403)
\end{exe}
\begin{exe}
\ex
\label{ex:8-25}
