...
\subsection{Basic \isi{tense}"=aspect categories}
\label{subsec:9-1-1}


There are (as shown in \figref{fig:9-1}) three non"=periphrastic categories that can be considered basic: \isi{Future}, \isi{Present} and \isi{Simple Past}.
...
\foreach \from/\to in {g/h, g/i, n/d, n/g}
                   \draw [->] (\from) -- (\to);
\end{tikzpicture}
\caption{Basic non"=periphrastic \isi{tense}"=aspect categories (bold type: inflectionally realised forms;
...
Within the im\isi{perfective}, the \isi{Future} category in Palula is the morphologically unmarked counterpart of the \isi{Present}, but it is also the periphrastically unmarked counterpart of the Past Im\isi{perfective} (see \sectref{subsec:9-1-6}). 
...
\isi{Future} is also often part of a~\isi{Conditional} expression as the apodosis verb (`will eat' in (\ref{ex:9-4})) and is sometimes used to express hopes and congratulations (as in (\ref{ex:9-5})), i.e., optative modality \citep[179]{bybeeetal1994}.
...
The \isi{Present} is also used for general statements (\ref{ex:9-10}) or sayings (\ref{ex:9-11}).
...
\subsection{Periphrastic \isi{tense}"=aspect categories}
\label{subsec:9-1-5}

What I refer to as periphrastic \isi{tense}"=aspect categories are those that are formed by a~\isi{finite} inflected verb and one or more auxiliaries. While the so very central aspectual distinctions ``at the heart of the \ili{NIA} verbal system'' \citep[262]{masica1991} very often are made morphologically and close to the verb stem, \isi{tense} and \isi{mood} are normally more peripherally marked functions in most modern IA languages. That is also the case with Palula, possibly with the exclusion of the morphologically marked \isi{Present} (as was pointed out above). 



Interesting to note is that this may be a~relatively recent (or early stage of) grammaticalisation. We find the same principle applied in the two closely related varieties \ili{Sauji} and \ili{Kalkoti}, i.e., the addition of elements to the basic aspectual categories to obtain \isi{tense} contrasts and to form perfects and pluperfects. Partly, however, we find other elements than those used in Palula, and in some cases, the former auxiliaries have become part of \isi{inflectional} morphology and even fused with aspect suffixes. In \ili{Kalkoti}, for instance, an \textit{s}-suffix (most certainly derived from the past \isi{copula} \textit{aas} `was, were') is added to the im\isi{perfective} (mostly corresponding to Palula \isi{Present}) to make it past im\isi{perfective}, \textit{čuṇ-uun-s} `was writing', and to the \isi{perfective} (mostly corresponding to Palula \isi{Simple Past}) to make it pluperfect, \textit{čuṇ-i-s} `had written' (the final consonant of the \isi{perfective} \textit{čuṇ-il} `wrote' has been dropped in phonological developments involving apocope and cluster reduction). There is generally speaking a~great variety among the \ili{Shina} varieties in how the less central TMA categories have been grammaticalised \citep[144--156]{liljegren2013}. \citet[38]{schmidt2004} provides examples of forms of `be', `go' as well as `come' as origins of such grammaticalisations in the respective varieties. 
...
\foreach \from/\to in {k/l, c/b, c/e, g/h, g/i, n/d, n/g}
                   \draw [->] (\from) -- (\to);
\end{tikzpicture}
\caption{Basic and periphrastic \isi{tense}"=aspect categories (bold type: inflectionally realised forms;
...
The Past Im\isi{perfective} is formed by means of the person"=inflected im\isi{perfective} verb stem (i.e., the future verb form) followed by the invariable marker \textit{de}. It has two main functions, as a~past habitual (\ref{ex:9-20}) and a~past progressive (\ref{ex:9-21}).
...
It is also used, instead of the \isi{Simple Past}, in some narratives, perhaps indicating the hearsay character~-- similar to the use of \textit{maní}~-- of the narrated content, as in (\ref{ex:9-26}), which is part of a~narrative where the \isi{Perfect} is used throughout.
...
There is also what appears to be a~parallel perfect form, used in a~very similar manner to the regular \isi{Perfect} in (\ref{ex:9-27})--(\ref{ex:9-29}). This is formed by combining the converb (\textsc{cv}) with the present \isi{tense} \isi{copula} \textit{hin-}.
...
The interesting thing, however, is that the ``regular'' \isi{Perfect} as described in the beginning of this section, seems to be missing altogether in the B. dialect. Instead, the converbal construction is widely used with in\isi{transitive} verbs, as in the examples (\ref{ex:9-27})--(\ref{ex:9-28}) and (\ref{ex:9-30})--(\ref{ex:9-31}), and probably marginally also with \isi{transitive} verbs, as in (\ref{ex:9-32})--(\ref{ex:9-33}) (although my B. data is too limited to the genre of narrative \isi{discourse} to provide a~clear indication).
...
Note that this construction, again focusing on the result rather than on the event itself, is based on a~\isi{transitive} im\isi{perfective} stem and therefore requires an~\isi{accusative agreement} pattern, while the agreement with the regular \isi{Perfect}, using the \isi{perfective} \isi{transitive} stem is \isi{ergative}. But there is also another (possibly more ``regular'' or lexically less restricted) perfect used in this dialect formed with converb+\textit{heensíl-} (the \isi{perfective} of the in\isi{transitive} verb \textit{háans}- `stay, live, exist') as in (\ref{ex:9-34}).
...
Even the A. variety has this construction (but with an~added present \isi{copula}), used in some instances of hearsay as in (\ref{ex:9-35}) or inferentially (by seeing water on the ground and subsequently reporting it to another person who has not been able to infer it), as in (\ref{ex:9-36}), but possibly still in free variation with the ``regular'' \isi{Perfect}:
...
\section{Non"=indicative \isi{finite} categories and their functions}
\label{sec:9-2}
...
Conditionality is essentially expressed with the protasis verb in a~\isi{Conditional} \isi{clause} in the \isi{Simple Past} (or one of the periphrastic categories built on the \isi{perfective}), while the apodosis verb may be in the \isi{Present} or the \isi{Future}, a ``split \isi{tense}/aspect reference'' not uncommon in languages overall (e.g., in \ili{Arabic}, \citealt[80]{dahl1985}). It is commonly, in such constructions, used together with an~additional element that can be analysed alternatively as a~subjunction, alternatively as an~\isi{auxiliary}. There are two such elements in Palula, one \textit{seentá} (B. \textit{síinta}) which is used to form constructions of assumed \isi{conditionality}, \textit{yhóolu seentá} `if he comes, when coming' (\ref{ex:9-53}) and another, \textit{heentá}, which usually carries a~hypothetical meaning, \textit{dítu heentá} `if you would give, were you to give' (\ref{ex:9-54}). Further details on \isi{conditionality} can be found in \sectref{subsec:13-4-4}. 
...
Whether or not the verb form coding the \isi{Obligative} should be seen as a~\isi{finite} or a~\isi{nonfinite} verb form is not as straightforward a~matter as it may first seem. It is analysable as both, and it is possible that we are witnessing a~still ongoing development from one to the other. 



The function of the \isi{Obligative} is to express obligation, need or predestination to carry out a~particular action. This is the agent"=oriented modality expressed in languages in general, though not necessarily inflectionally \citep[177--187]{bybeeetal1994}. Often in IA languages this particular semantics is connected with what \citet[322]{masica1991} refers to as a \textit{future \isi{passive} participle (of obligation)}, which behaves like an~\isi{adjective}, but may in some languages be formally identical to a~verbal \isi{noun}, as is the case with the \ili{Urdu}"=Hindi infinitive formed with \textit{-nā}. 
...
It seems the agent is generally avoided with obligative \isi{transitive} verbs, and as such is also becoming an~alternative way of expressing a~\isi{passive} meaning (cf. \sectref{subsec:8-5-2}), with sense of ability (or inability) rather than of obligation. I would still hold that \isi{Obligative} is primarily modality and only secondarily voice. Possibly related to the degree of obligation is an~alternation between the coding of the agent in the \isi{nominative} (\ref{ex:9-59}) versus the \isi{genitive} coding we saw above (\ref{ex:9-55}).
...
It is, however, not restricted to narrative \isi{discourse}. It may be used in any everyday conversation, as in (\ref{ex:9-62}) with the \isi{Perfect}. Also in this utterance it is only after the first \isi{finite} verb form that the hearsay marker/\isi{auxiliary} occurs.
...
Related to this issue, is the use of the quotation marker \textit{thaní}, with a~very similar etymology, which may develop into an~\isi{auxiliary}. Triggered by the verb"=final word order, it may become restricted to the position after the \isi{finite} verb and become narrowed down to this particular verb form (now it is alternatively expressed with other verb forms). In the process it may also become a~marker of a~more grammaticalised inferentiality category, as such contrasting with \isi{Hearsay}, rather than now as chiefly a~marker of direct quotation/perception (see \sectref{subsec:13-5-1}). In (\ref{ex:9-63}) it has taken on a~function beyond mere quotation.
...
The grammaticalisation of \isi{Hearsay} as well as other types of inferentiality is documented for a~number of other languages in the region, especially in the geographically adjacent \ili{Khowar} and \ili{Kalasha}, where it is particularly well"=developed and is an~integral part of verb morphology \citep{bashir1996}. 


\section{Non\isi{finite} forms and their functions}
\label{sec:9-3}

Most of the \isi{nonfinite} verb forms have implications for syntax and are hard to define functionally without discussing syntactic structure. The following sections therefore only aim at giving a~basic characterisation of their function; where to find them in the grammatical system and what their further \isi{inflectional} properties are, wherever the latter is relevant. For further details in this regard, the cross"=references given at the end of each subsection should be consulted. 


\subsection{\isi{Converb} (\isi{conjunctive participle}) }
\label{subsec:9-3-1}


The \isi{Converb} (defined by \citet{haspelmath1995} as ``a \isi{nonfinite} verb form whose main function is to mark adverbial subordination'') is essentially a~\isi{perfective} adverbial participle (cf. the \isi{Copredicative Participle}, \sectref{subsec:9-3-5}, which is an~im\isi{perfective} adverbial participle). It carries an~ap\isi{proximate} meaning of `having done, etc.' and is, as in many other IA languages \citep[323, 397--401]{masica1991}, where it often is referred to as a~\isi{conjunctive participle},\footnote{The identification of the corresponding verb form in \ili{Kohistani} \ili{Shina} (otherwise referred to as a~\isi{conjunctive participle}) with the converb, according to \citeauthor{haspelmath1995}'s (\citeyear{haspelmath1995}) definition, is carried through in \citet{schmidt2003}.} a~characteristic, very frequently occurring and syntactically important \isi{nonfinite} verb form. It is (as \citeauthor{haspelmath1995}'s definition also spells out) especially important in adverbial subordination and in expressing \isi{perfective} sequentiality, as illustrated in (\ref{ex:9-64}) (see \sectref{subsec:13-3-1} and \sectref{subsec:13-4-1} for further details).
...
Since the \isi{Perfective Participle} is formally identical to the \isi{Simple Past} (and as such has become a~\isi{finite} verb form), it only occurs marginally in what must be interpreted as a~\isi{nonfinite} function. When it does, such as in the (\isi{perfective}) \isi{nominalisation} below, it can also (but only optionally) be inflected for case, \isi{genitive} in (\ref{ex:9-65}) and \isi{oblique} in (\ref{ex:9-66}).
...
The \isi{Verbal Noun} is in many ways the im\isi{perfective} counterpart of the \isi{Perfective Participle}, although their respective usages do not exactly mirror each other. The verbal \isi{noun} is the form that most clearly in the eyes/ears of educated mother"=tongue speakers essentialises the verb, the form first given or normally used when talking metalinguistically about functions and meanings of individual verbs. It is also regarded as the most natural choice as a~citation form in word lists. 


Although textually not as frequent as the \isi{Converb}, it is a~category used in a~number of dependent clauses, such being the \isi{predicate} in many \isi{complement} clauses (\ref{ex:9-69}) (for details, see \sectref{sec:13-5}) as well as in some clauses with adverbial functions (\sectref{sec:13-4}).
...
\sectref{subsec:12-2-7}. In this combination the \isi{Infinitive} tends to be more or less deaccented and phonologically
fused with the matrix \isi{clause} \isi{finite} verb.
